...
public class ListTester {
	...
	private void runTests() {
		...
		//2-element to changed 2-element via set()
		 // testTwoElementList(AB_set0C_CB, "ABC_set0C_CB", LIST_CB, STRING_CB);
		//  testTwoElementList(AB_set1C_AC, "ABC_set1C_AC", LIST_AC, STRING_AC);
		  ...
		  //testTwoElementList(ABC_iterRemoveAfterNextC_AB,"ABC_iterRemoveAfterNextC_AB", LIST_AB, STRING_AB); //3-element to changed
		//  3-element via set() 
...
private Scenario<Integer> A_removeFirst_emptyList = () ->  A_removeFirst_emptyList();
	...
	/** Scenario: [A] -> add(0,A) -> [A,A] 
	 * @return [A,A] after add(0,A)
	 */
	private IndexedUnsortedList<Integer> A_add0A_AA() {
		IndexedUnsortedList<Integer> list = emptyList_addToFrontA_A(); 
		list.add(0, ELEMENT_A);
		return list;
	}
	private Scenario<Integer> A_add0A_AA = () -> A_add0A_AA();
	
	/** Scenario: [A] -> add(1,A) -> [A,A] 
	 * @return [A,A] after add(1,A)
	 */
	private IndexedUnsortedList<Integer> A_add1A_AA() {
		IndexedUnsortedList<Integer> list = emptyList_addToFrontA_A(); 
		list.add(1, ELEMENT_A);
		return list;
	}
	private Scenario<Integer> A_add1A_AA = () -> A_add1A_AA();
	
	/** Scenario: [B] -> add(0,B) -> [B,B] 
	 * @return [B,B] after add(0,B)
	 */
	private IndexedUnsortedList<Integer> B_add0B_BB() {
		IndexedUnsortedList<Integer> list = A_set0B_B(); 
		list.add(0, ELEMENT_B);
		return list;
	}
	private Scenario<Integer> B_add0B_BB = () -> B_add0B_BB();
	...
	/** Scenario: [ABC] -> removeA -> [BC] 
	 * @return [BC] after removeA
	 */
	...
	/** Scenario: [ABC] -> removeB -> [AC] 
	 * @return [AC] after removeB
	 */
	...
	/** Scenario: [ABC] -> removeC -> [AB] 
	 * @return [AB] after removeC
	 */
	...
	/** Scenario: [ABC] -> remove1 -> [AC] 
	 * @return [AC] after remove1
	 */
	...
	private void testEmptyList(Scenario<Integer> scenario, String scenarioName) {
		...
		try {
			...
			//an iterator after one call to next() immediately followed by a call to remove()
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_testIndexOf" + contentsString.charAt(0), testIndexOf(scenario.build(), contents[0], 0));
			printTest(scenarioName + "_testIndexOf" + contentsString.charAt(0), testIndexOf(scenario.build(), contents[1], 1));
			printTest(scenarioName + "_testIndexOfX", testIndexOf(scenario.build(), ELEMENT_X, -1));
			printTest(scenarioName + "_testRemoveNeg1", testRemoveIndex(scenario.build(), -1, null, Result.IndexOutOfBounds));
			printTest(scenarioName + "_testRemove0", testRemoveIndex(scenario.build(), 0, contents[0], Result.MatchingValue));
...
//			an iterator after one call to next(), a call to remove(), and finally, another call to next()
			...
			printTest(scenarioName + "_iterNextRemove_testIterNext", testIterNext(iterAfterRemove(iterAfterNext(scenario.build(), 1)),null, Result.NoSuchElement));
			printTest(scenarioName + "_iterNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 1)), Result.IllegalState));
			printTest(scenarioName + "_iterNextNext_testIterHasNext", testIterHasNext(iterAfterNext(scenario.build(), 1), Result.True));
//			an iterator after two consecutive calls to next() immediately followed by a call to remove()
			printTest(scenarioName + "_iterNextNext_testIterRemove", testIterRemove(iterAfterNext(scenario.build(), 1), Result.NoException));
//			an iterator after one call to next(), a call to remove(), and finally, another call to next()
			printTest(scenarioName + "_iterNextRemove_testIterHasNext", testIterHasNext(iterAfterRemove(iterAfterNext(scenario.build(), 1)), Result.False));
			printTest(scenarioName + "_iterNextRemove_testIterNext", testIterNext(iterAfterRemove(iterAfterNext(scenario.build(), 1)), null, Result.NoSuchElement));
			printTest(scenarioName + "_iterNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 1)), Result.IllegalState));
